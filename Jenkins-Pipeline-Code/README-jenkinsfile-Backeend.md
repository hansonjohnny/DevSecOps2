...
| **AWS CLI** (installed on Jenkins agent) | Authenticating with ECR |
...
All sensitive values are injected from **Jenkins Credentials** — never hardcoded.
...
**Directory:** `Todo-app/backend`

Runs the SonarQube Scanner against the backend source code and reports results to the configured SonarQube server.

```
Project Key:  cloud-native-backend
Project Name: cloud-native-backend
```
...
> **Note:** The enforcement scan's `--exit-code` is currently set to `0`, meaning it will not fail the build. Change to `--exit-code 1` to enforce a hard stop on HIGH/CRITICAL filesystem findings.
...
> **Note:** Same caveat as above — set `--exit-code 1` to enforce hard failures on critical image vulnerabilities.
